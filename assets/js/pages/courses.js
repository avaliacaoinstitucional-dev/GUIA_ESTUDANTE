...
function render(){const filtered=filterCourses(GUIDE_DATA.courses,{q:els.q.value,area:els.area.value,school:els.school.value}); els.count.textContent=`${filtered.length} curso(s) encontrado(s)`; els.list.innerHTML=filtered.map(courseCard).join('') || '<div class="card"><h3>Nenhum curso encontrado</h3><p>Tente limpar os filtros ou usar outra palavra.</p></div>'}
